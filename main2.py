...
TEST_STEPS = 3600           # Steps for test phase after training
...
def train_model(epochs, gui=False):
    ...
    global total_steps, epsilon, cumulative_reward
    total_steps = 0
    epsilon = EPSILON_START
    cumulative_reward = 0.0

    for episode in range(NUM_EPISODES):
        # Setup phase logger for this episode
        phase_logger = logging.getLogger(f'phase_logger_episode_{episode + 1}')
        phase_handler = logging.FileHandler(f'phase_log_episode_{episode + 1}.txt')
        ...
        # Start SUMO simulation for each episode
        try:
            ...
            sys.exit(f"Error starting SUMO: {e}")
        except traci.TraCIException as e:
        ...
        step = 0
        while step < TOTAL_STEPS_PER_EPISODE:
            # Training logic from the second block...
            ...
            if should_change_phase(current_simulation_step):
                ...
                phase_logger.info(f"Step {step}, Green Light for {roads_with_green}, Duration {duration // 10} seconds")
            
            try:
                traci.simulationStep()
            except traci.TraCIException as e:
                print(f"Simulation step failed at step {step}: {e}")
                logging.error(f"Simulation step failed at step {step}: {e}")
                break
            
            new_state = get_state()
            passed_vehicles = get_passed_vehicles()
            reward = get_reward(new_state, passed_vehicles, action if should_change_phase(current_simulation_step) else prev_action, prev_action)
            episode_reward += reward
            cumulative_reward += reward
            
            current_action = action if should_change_phase(current_simulation_step) else prev_action
            update_model(state, current_action, reward, new_state)
            
            if total_steps % TARGET_UPDATE == 0:
                target_net.load_state_dict(policy_net.state_dict())
                logging.info(f"Target network updated at step {total_steps}")
            
            epsilon = max(EPSILON_END, epsilon * EPSILON_DECAY)
            
            if step % 100 == 0:
                total_waiting_time = get_waiting_time()
                print(f"Episode {episode + 1}, Step {step}, Total Steps {total_steps}, "
                      f"Duration: {current_phase_duration // 10}, Reward: {reward:.2f}, "
                      f"Episode Reward: {episode_reward:.2f}, Cumulative Reward: {cumulative_reward:.2f}, "
                      f"Waiting Time: {total_waiting_time:.2f}, Epsilon: {epsilon:.4f}")
                logging.info(f"Episode {episode + 1}, Step {step}, Total Steps {total_steps}, "
                             f"Duration: {current_phase_duration // 10}, Reward: {reward:.2f}, "
                             f"Episode Reward: {episode_reward:.2f}, Cumulative Reward: {cumulative_reward:.2f}, "
                             f"Waiting Time: {total_waiting_time:.2f}, Epsilon: {epsilon:.4f}")
            ...
            queue_history.append(sum(state[:-1]))  # Sum of all queue lengths
            waiting_time_history.append(get_waiting_time())
            
            prev_action = action if should_change_phase(current_simulation_step) else prev_action
            step += 1
        ...
        plt.title(f"Episode {episode + 1} Queue Length")
        plt.legend()
        plt.grid(True)
        plt.savefig(f"plots/episode_{episode + 1}_queue.png", bbox_inches='tight', dpi=300)
        plt.close()

        plt.figure(figsize=(10, 6))
        plt.plot(step_history, waiting_time_history, label="Waiting Time")
        plt.xlabel("Steps")
        plt.ylabel("Waiting Time (seconds)")
        plt.title(f"Episode {episode + 1} Waiting Time")
        plt.legend()
        plt.grid(True)
        plt.savefig(f"plots/episode_{episode + 1}_waiting.png", bbox_inches='tight', dpi=300)
        plt.close()

        # Close SUMO connection for this episode
        try:
            traci.close()
        except traci.TraCIException as e:
            logging.warning(f"Error closing TraCI connection: {e}")
...
# -------------------------
# Step 9: Test Phase to Observe Chosen Durations
# -------------------------

print("\n=== Starting Test Phase to Observe Model's Phase Durations ===")
logging.info("Starting test phase to observe phase durations.")
...
try:
    traci.start(get_sumo_config(gui=True))
    traci.gui.setSchema("View #0", "real world")
    for _ in range(10):
        traci.simulationStep()
except traci.FatalTraCIError as e:
